...
from lstm_config import get_config, print_usage
...
class RNN(object):
    ...
    def _build_trainable(self):
        ...
        with tf.variable_scope("Weights", reuse=tf.AUTO_REUSE):
            ...
            # Weight for input gate, x vector
            self.Wix = tf.get_variable(
                "Wix", (num_unit, self.num_words), tf.float32, sample_initializer)
            # Weight for input gate, previous time vector
            self.Wih = tf.get_variable(
                "Wih", (num_unit, num_unit), tf.float32, initializer)
            # Weight for input gate, image vector
            self.Wii = tf.get_variable(
                "Wii", (num_unit, self.img_shp), tf.float32, initializer)

            # Weight for forget gate, x vector
            self.Wfx = tf.get_variable(
                "Wfx", (num_unit, self.num_words), tf.float32, sample_initializer)
            # Weight for forget gate, previous time vector
            self.Wfh = tf.get_variable(
                "Wfh", (num_unit, num_unit), tf.float32, initializer)
            # Weight for forget gate, image vector
            self.Wfi = tf.get_variable(
                "Wfi", (num_unit, self.img_shp), tf.float32, initializer)

            # Weight for output gate, x vector
            self.Wox = tf.get_variable(
                "Wox", (num_unit, self.num_words), tf.float32, sample_initializer)
            # Weight for output gate, previous time vector
            self.Woh = tf.get_variable(
                "Woh", (num_unit, num_unit), tf.float32, initializer)
            # Weight for output gate, image vector
            self.Woi = tf.get_variable(
                "Woi", (num_unit, self.img_shp), tf.float32, initializer)
            
            # Weight for candidate cell state, x vector
            self.Wgx = tf.get_variable(
                "Wgx", (num_unit, self.num_words), tf.float32, sample_initializer)
            # Weight for memory, previous time vector
            self.Wgh = tf.get_variable(
                "Wgh", (num_unit, num_unit), tf.float32, initializer)
            # Weight for memory, image vector
            self.Wgi = tf.get_variable(
                "Wgi", (num_unit, self.img_shp), tf.float32, initializer)
    ...
    def _build_train_model(self):
        ...
        def cond(t, ct, st, pred, logits):
            return tf.logical_not(tf.reduce_all(tf.equal(self.x[t], self.end)))

        def timestep(t, ct, st, pred, logits):
            # Input gate
            it = tf.sigmoid(
                tf.matmul(self.Wix, self.x[t]) + tf.matmul(self.Wih, st) + tf.matmul(self.Wii, self.img))
            # Forget gate
            ft = tf.sigmoid(
                tf.matmul(self.Wfx, self.x[t]) + tf.matmul(self.Wfh, st) + tf.matmul(self.Wfi, self.img))
            # Output gate
            ot = tf.sigmoid(
                tf.matmul(self.Wox, self.x[t]) + tf.matmul(self.Woh, st) + tf.matmul(self.Woi, self.img))
            # Candidate cell state
            gt = tf.tanh(
                tf.matmul(self.Wgx, self.x[t]) + tf.matmul(self.Wgh, st) + tf.matmul(self.Wgi, self.img))
            # Update cell state
            ct = ct * ft + gt * it
            # New cell state
            st = tf.tanh(ct) * ot
            ...
            return [t + 1, ct, st, pred, logits]

        t = tf.constant(0)
        # Initialize candidate cell state
        ct = tf.zeros([self.config.num_unit, 1])
        # Initialize final cell state
        st = tf.zeros([self.config.num_unit, 1])
        # Initialize prediction matrix
        pred = tf.zeros([1, 1])
        logits = tf.zeros([1, self.num_words, 1])

        # Loop through words in sentence, outputting predicted caption
        self.t, ct, st, self.sentence, self.logits = tf.while_loop(cond, timestep, [t, ct, st, pred, logits], [t.shape, ct.shape, st.shape, tf.TensorShape([None, 1]), tf.TensorShape([None, self.num_words, 1])])
    ...
    def _build_test_model(self):
        ...
        def cond(t, ct, st, pred, yt):
            return tf.logical_not(tf.reduce_all(tf.equal(yt, self.end)))

        def timestep(t, ct, st, pred, yt):
            # Input gate
            it = tf.sigmoid(
                tf.matmul(self.Wix, yt) + tf.matmul(self.Wih, st) + tf.matmul(self.Wii, self.img))
            # Forget gate
            ft = tf.sigmoid(
                tf.matmul(self.Wfx, yt) + tf.matmul(self.Wfh, st) + tf.matmul(self.Wfi, self.img))
            # Output gate
            ot = tf.sigmoid(
                tf.matmul(self.Wox, yt) + tf.matmul(self.Woh, st) + tf.matmul(self.Woi, self.img))
            # Candidate cell state
            gt = tf.tanh(
                tf.matmul(self.Wgx, yt) + tf.matmul(self.Wgh, st) + tf.matmul(self.Wgi, self.img))
            # Update cell state
            ct = ct * ft + gt * it
            # New cell state
            st = tf.tanh(ct) * ot
        ...
        # Loop through words in sentence, outputting predicted caption
        t, ct, st, self.pred, yt = tf.while_loop(cond, timestep, [t, ct, st, pred, yt], [t.shape, ct.shape, st.shape, tf.TensorShape([None, 1]), yt.shape])
