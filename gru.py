...
from gru_config import get_config, print_usage
...
class RNN(object):
    ...
    def _build_trainable(self):
        ...
        with tf.variable_scope("Weights", reuse=tf.AUTO_REUSE):
            ...
            # Weight for update gate, x vector
            self.Wzx = tf.get_variable(
                "Wzx", (num_unit, self.num_words), tf.float32, sample_initializer)
            # Weight for update gate, previous time vector
            self.Wzh = tf.get_variable(
                "Wzh", (num_unit, num_unit), tf.float32, initializer)
            # Weight for update gate, image vector
            self.Wzi = tf.get_variable(
                "Wzi", (num_unit, self.img_shp), tf.float32, initializer)

            # Weight for reset gate, x vector
            self.Wrx = tf.get_variable(
                "Wrx", (num_unit, self.num_words), tf.float32, sample_initializer)
            # Weight for reset gate, previous time vector
            self.Wrh = tf.get_variable(
                "Wrh", (num_unit, num_unit), tf.float32, initializer)
            # Weight for reset gate, image vector
            self.Wri = tf.get_variable(
                "Wri", (num_unit, self.img_shp), tf.float32, initializer)

            # Weight for memory, x vector
    ...
    def _build_train_model(self):
        ...
        def cond(t, st, pred, logits):
            return tf.logical_not(tf.reduce_all(tf.equal(self.x[t], self.end)))

        def timestep(t, st, pred, logits):
            # Update gate
            zt = tf.sigmoid(
                tf.matmul(self.Wzx, self.x[t]) + tf.matmul(self.Wzh, st) + tf.matmul(self.Wzi, self.img))
            # Reset (forget) gate
            rt = tf.sigmoid(
                tf.matmul(self.Wrx, self.x[t]) + tf.matmul(self.Wrh, st) + tf.matmul(self.Wri, self.img))
            # Determine what information is to be kept/updated
            ht = tf.tanh(
                tf.matmul(self.Wx, self.x[t]) + rt * tf.matmul(self.Wh, st) + tf.matmul(self.Wi, self.img))
            # Update memory with new data
            st = (tf.ones(zt.shape) - zt) * ht + zt * st
        ...
        self.t, st, self.sentence, self.logits = tf.while_loop(cond, timestep, [t, st, pred, logits], [t.shape, st.shape, tf.TensorShape([None, 1]), tf.TensorShape([None, self.num_words, 1])])
    ...
    def _build_test_model(self):
        ...
        def cond(t, st, pred, yt):
            return tf.logical_not(tf.reduce_all(tf.equal(yt, self.end)))

        def timestep(t, st, pred, yt):
            # Update gate
            zt = tf.sigmoid(
                tf.matmul(self.Wzx, yt) + tf.matmul(self.Wzh, st) + tf.matmul(self.Wzi, self.img))
            # Reset (forget) gate
            rt = tf.sigmoid(
                tf.matmul(self.Wrx, yt) + tf.matmul(self.Wrh, st) + tf.matmul(self.Wri, self.img))
            # Determine what information is to be kept/updated
            ht = tf.tanh(
                tf.matmul(self.Wx, yt) + rt * tf.matmul(self.Wh, st) + tf.matmul(self.Wi, self.img))
            # Update memory with new data
            st = (tf.ones(zt.shape) - zt) * ht + zt * st
        ...
        # Loop through words in sentence, outputting predicted caption
        t, st, self.pred, yt = tf.while_loop(cond, timestep, [t, st, pred, yt], [t.shape, st.shape, tf.TensorShape([None, 1]), yt.shape])
